/* eslint-env worker */
/* global FS, callMain */
/* QPDF.js worker — Source: https://github.com/j3k0/qpdf.js (Apache-2.0), lightly modified.
   This file is self-hosted (same-origin) so `new Worker(...)` is allowed by the browser.
   It then loads the heavy emscripten/WASM build (lib/qpdf.js + lib/qpdf.wasm) straight
   from jsDelivr via importScripts()/fetch(), which — unlike the Worker() constructor —
   is permitted cross-origin as long as the response allows CORS (jsDelivr always does). */

const basePath = 'https://cdn.jsdelivr.net/npm/qpdf.js@2.0.0/src/';
let errorOrWarning = '';
...
function decodeQPDFExc(addr) {
    try {
        ...
        const errorPatterns = [
            { type: 'file_error', markers: ["can't find", "error", "failed", "unrecognized", "invalid"] },
            { type: 'object_error', markers: ["expected", "object", "offset"] }
        ];
